    ...
    if (capture.isOpened()) {
      while (running && capture.read(captureMatrix)) {
        ...
        if (option.equals("Normal Color")) {
          ...
          bufferedImage = new BufferedImage(width, height, BufferedImage.TYPE_3BYTE_BGR);
          final Mat grayMat = captureMatrix.clone();
          outputMatrix = captureMatrix.clone();
          Imgproc.cvtColor(grayMat, grayMat, Imgproc.COLOR_RGB2GRAY);
          final Mat sobelX = new Mat(width, height, type);
          final Mat sobelY = new Mat(width, height, type);
          Imgproc.Sobel(grayMat, sobelX, type, 1, 0);
          Imgproc.Sobel(grayMat, sobelY, type, 0, 1);
          Core.addWeighted(sobelX, 0.5, sobelY, 0.5, 1, grayMat);
          Mat circles = new Mat();
          Imgproc.HoughCircles(grayMat, circles, Imgproc.CV_HOUGH_GRADIENT, 1, 60, 200, 50, 0,200);
          if (circles.cols() > 0) {
          ...
          Core.addWeighted(sobelX, 0.5, sobelY, 0.5, 1, grayMat);
          Mat circles = new Mat();
          Imgproc.HoughCircles(grayMat, circles, Imgproc.CV_HOUGH_GRADIENT, 1, 60, 200, 40, 0,200);
